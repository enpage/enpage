...
const createRemScale =
  (
    base: number | string = "1rem",
    ratio?: Parameters<typeof modularScale>[2],
  ) =>
  (steps: number) =>
    `${modularScale(steps, base, ratio)}`;

const spaceScale = createRemScale("0.15rem");
const fontSizeScale = createRemScale("1rem", 1.25);
const lineHeightScale = createRemScale("1rem", 1.25);
const sizeScale = createRemScale("0.125rem", 2);
...
export const properties = defineProperties({
  conditions: {
    ...
    // tablet: { "@container": `${respContainerName} (min-width: 448px)` },
    // desktop: { "@container": `${respContainerName} (min-width: 768px)` },
  },
